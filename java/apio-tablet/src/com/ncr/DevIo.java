...
import com.ncr.tablet.AsrTabletConfig;
import com.ncr.tablet.LineMapFactory;
...
abstract public class DevIo extends Struc {
    ...
    static LineMapFactory lf = new LineMapFactory();

    static LineMapIf lMap = lf.getLineMap(AsrTabletConfig.getInstance().isEnable(),"PrtLine");
    ...
    public static void tpmPrint(int dev, int lfs, String data) {
            ...
            logger.debug("tpmPrint: >" + data + "< bmpSequence: " + tra.bmpSequence + " - asrTablet.enabled: " + AsrTabletConfig.getInstance().isEnable());
            if (name != null) {
                ReceiptManager.getInstance().bitmap(name);
                return;
            }
    }
